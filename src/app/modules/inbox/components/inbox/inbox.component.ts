...
export class InboxComponent implements OnInit {
  ...
  public columnsHeader = [
    ...
    { name: 'actions', display: 'Actions', size: 15 }
  ];
  ...
  private fillInboxData(inbox: Inbox) {
    ...
    this.inboxListData = this.sortLatestByDate(this.inboxData.model);
  ...
  public onDateChange(event: any): void {
    if (event.startDate) {
      this.dateFrom = event.startDate._d.toLocaleString();
      this.dateTo = event.endDate._d.toLocaleString();
      this.loadInbox();
    }
  }
  ...
  private sortLatestByDate(dataList: any[]): any[] {
    return dataList.slice().sort(function(el1, el2) {
      if (el1.uploadDate < el2.uploadDate) {
        return 1
      }
      if (el1.uploadDate > el2.uploadDate) {
        return -1
